...
public class KNNOriginal {
    ...
    private final double alpha = 0.5;    // Fehlerrate fuer Backprobagation
    private final int maxIter = 1;      // Anzahl Iterationen bei Fehlerminimierung
    private final int maxEpoche = 10;// Anzahl Iterationen bei Fehlerminimierung
    ...
    public KNNOriginal(int anzahlEingabewerte, int[] anzahlKnotenProHiddenSchicht) {
        ...
        for (int l = 0; l < m; l++) {
            for (int i = 0; i < netz[l].length; i++) {
                knotenNr = netz[l][i];
                bias[knotenNr] = i == 0 && l < m - 1;// der erste Knoten einer Schicht wird bias (aussnahme in der ausgabeschicht)
            }
        }
...
//	}
    ...
    private void deltaAusgabeSchicht(double klasse) {
        ...
        for (int nrj = 0; nrj < netz[ausgabeSchicht].length; nrj++) {
            ...
            delta[j] = ableitungAktivierungsFunktion(in[j]) * (a[j] - klasse);
        }
    }
